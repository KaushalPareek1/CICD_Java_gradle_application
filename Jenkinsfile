pipeline {
  ...
  environment {
    ...
    DOCKER_REGISTRY = "13.232.186.61:8083"  // Replace with your actual registry address
  }
                    ...
                    withCredentials([usernamePassword(credentialsId: 'docker-host', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASSWORD')]) {
                      dir('kubernetes/') {
                        sh '''
                         ...
                         curl -u admin:$docker_password http://13.232.186.61:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                          '''
                          }
                    }
                }
